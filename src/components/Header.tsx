...
const Header = () =>{
    ...
    const { isAuthenticated, user, logout, empresaUuid } = useAuth();
        ...
        <header className="bg-stone-700 w-full h-[150px] relative top-0 flex items-end">
            ...
            <div className="flex gap-10 text-2xl text-white mb-[40px] w-4/6 justify-center">
                ...
                {isAuthenticated && (
                    <>
                        ...
                        {empresaUuid && (
                            <p onClick={() => navigate(`/${empresaUuid}/catalogo`)} className="hover:underline cursor-pointer">Catálogo da empresa</p>
                        )}
                        <p onClick={() => navigate("/pedidos")} className="hover:underline cursor-pointer">Pedidos</p>
                    </>
                )}
            </div>
            <div className="w-1/6  flex justify-end items-center">
                {!isAuthenticated ? (
                    ...
                    <div onClick={handleLogout} className="cursor-pointer px-5 hover:bg-red-900 flex justify-center items-center gap-1 text-2xl text-white mb-[30px] mr-[40px] bg-[#A8201A]  rounded-full px-2 py-2">
                        <p>Logout</p>
                    </div>
                )}
            </div>
