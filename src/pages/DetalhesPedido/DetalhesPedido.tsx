...
import { Pedido } from "../../models/pedido.model"; // Import Pedido model
import TabelaItensPedido             from "./components/TabelaItensPedido";

const DetalhesPedido = () => {

    const [pedido, setPedido] = useState<Pedido | null>(null) // Type the state
    const {pedidoId} = useParams<{ pedidoId: string }>() // Type useParams

    const { pedidoService } = useServices(); // Use pedidoService

    const carregarPedido = async () => {
        if (!pedidoId) return;
        try {
            const res = await pedidoService.getPedidoByUuid(pedidoId); // Use the new service method
            setPedido(res.data);             
        } catch (err) {
    ...
    const concluir = async () => {
        ...
        try {
            ...
            carregarPedido(); // Recarregar o pedido para atualizar o status
        } catch (error) {
                ...
                <div className="mx-[100px] mt-[50px] space-y-[50px] pb-[40px]">
                    <div className="space-y-10">
                        ...
                        <div className="flex pb-5 pt-10 rounded-2xl bg-white border-[#A8201A] border-2 w-full relative">
                            ...
                            {pedido.concluido === false && ( // Only show conclude button if not concluded
                                <div 
                                    onClick={concluir}
                            ...
                            <div className="flex w-1/3">
                                <div className="text-xl font-semibold h-full w-1/3">
                                    <h2 className="border p-1 border-l-0 h-1/6 flex items-center justify-center w-full">UUID</h2>
                                    <h2 className="border p-1 border-l-0 h-1/6 border-t-0 flex items-center justify-center w-full">Nome do Pedido</h2>
                            ...
                            <div className="flex w-2/3">
                                ...
                                <div className="text-md h-full w-2/3">
                                    ...
                                    <h2 className="border px-1 border-t-0 border-l-0 border-r-0 h-1/7 flex items-center justify-center w-full">
                                        {pedido?.endereco_entrega?.complemento || 'N/A'}
                                    </h2>
